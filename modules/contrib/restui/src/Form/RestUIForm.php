...
class RestUIForm extends ConfigFormBase {
   ...
   */
  public function __construct(ConfigFactory $config_factory, ModuleHandler $module_handler, array $authenticationProviders, array $formats, ResourcePluginManager $resourcePluginManager, UrlGeneratorInterface $url_generator, RouteBuilder $routeBuilder) {
  ...
  public function validateForm(array &$form, array &$form_state) {
    ...
    foreach ($form_state['values']['methods'] as $method => $values) {
      if ($values[$method] == 1) {
        $method_checked = TRUE;
        ...
        if (empty($formats)) {
          form_set_error('methods][' . $method . '][settings][formats' , $form_state,
            $this->t('At least one format must be selected for method !method.', array('!method' => $method)));
        }
        if (empty($auth)) {
          form_set_error('methods][' . $method . '][settings][auth' , $form_state,
            $this->t('At least one authentication method must be selected for method !method.', array('!method' => $method)));
        }
      }
    }
    if (!$method_checked) {
      form_set_error('methods', $form_state, $this->t('At least one HTTP method must be selected'));
    }
  }
